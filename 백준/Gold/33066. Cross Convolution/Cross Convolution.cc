...
void solve() {
    ...
    auto get_sum = [&](int r1, int c1, int r2, int c2) -> ll {
        // P[r2+1][c2+1] - P[r1][c2+1] - P[r2+1][c1] + P[r1][c1]
        // 여기서 P의 인덱스들은 A의 인덱스보다 1 큽니다.
    ...
    for (int i = 0; i < N_prime; ++i) { // B 행 인덱스
        for (int j = 0; j < M_prime; ++j) { // B 열 인덱스
            ...
            ll row_sum = get_sum(R_center, c_start, R_center, c_start + K - 1);

            // 2. 중심 열의 합 (c1=c2=C_center, r1=r_start, r2=r_start+K-1)
            ll col_sum = get_sum(r_start, C_center, r_start + K - 1, C_center);
